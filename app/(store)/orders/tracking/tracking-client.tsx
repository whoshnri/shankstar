...
import { CheckCircle, Search, Package, Truck, CheckCircle2, XCircle } from 'lucide-react';
...
export default function TrackingClient({ initialOrder }: TrackingClientProps) {
                ...
                <div className="space-y-4">
                  {order.items.map((item: any) => (
                    <div key={item.id} className="flex justify-between items-center text-xs p-4 bg-secondary/20 border border-border/50">
                      <div>
                        <p className="font-bold uppercase tracking-widest">{item.product?.name ?? 'Product'}</p>
                        <p className="text-muted-foreground mt-1 uppercase tracking-tighter opacity-70">
                          Qty: {item.quantity}
                        </p>
                      </div>
